...
if [ -z "$VPN_IPSEC_PSK" ] && [ -z "$VPN_USER" ] && [ -z "$VPN_PASSWORD" ]; then
  ...
  echo ""
  echo " +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+"
  echo " | L2TP/IPSec PSK VPN Setup Script By Aung Thu Myint |"
  echo " +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+"
  echo ""
  echo -n " [#] Enter IPSecPSK VPN UserName : "
  read VPN_USER
  echo ""
  echo -n " [#] Enter IPSecPSK VPN Password : "
  read VPN_PASSWORD
  echo ""
  echo " +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+"
  echo ""
  ...
  echo "[+] Generating Random PSK Successfully."
fi

if [ -z "$VPN_IPSEC_PSK" ] || [ -z "$VPN_USER" ] || [ -z "$VPN_PASSWORD" ]; then
  exiterr "[!] All VPN Credentials Must Be Specified. Edit The Script & Re-Enter Them."
fi

if printf '%s' "$VPN_IPSEC_PSK $VPN_USER $VPN_PASSWORD" | LC_ALL=C grep -q '[^ -~]\+'; then
  exiterr "[!] Not Contain Non-ASCII Characters."
fi

case "$VPN_IPSEC_PSK $VPN_USER $VPN_PASSWORD" in
  *[\\\"\']*)
    exiterr "[!] Not Contain These Special Characters : \\ \"'"
    ;;
esac
...
echo "[+] Compiling And Installing Libreswan, Please Wait ..."
...
cat <<EOF
 ...
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
 | L2TP/IPSec PSK VPN Setup Script By Aung Thu Myint |
 +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

 [#]  Server IP : $public_ip
 [#]  IPSecPSK  : $VPN_IPSEC_PSK
 [#]  UserName  : $VPN_USER
 [#]  Password  : $VPN_PASSWORD

 If You Want To Add New User & Del User
 
 Go To This Directory /etc/ppp/chap-secrets
